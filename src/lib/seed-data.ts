...
export async function seedDemoData(prisma: PrismaClient) {
  ...
  const invoiceData = [
    ...
    // Billed under the advisory firm — the receipt register is kept firm-wise.
    { invoiceNumber: "SAS/26-27/001", organizationId: advisoryOrg.id, clientId: meera.id, description: "ITR filing & advisory – AY 2026-27", amount: 8000, taxRate: 18, status: "Paid", issueDate: daysFromNow(-12), dueDate: daysFromNow(3), paidDate: daysFromNow(-8), receiptNumber: "SAS/26-27/R001", paymentMode: "UPI", transactionRef: "UPI 618834412907" },
  ...
  const packetData = [
    { inwardNumber: "IN-2627-001", receivedFrom: "Rohan Mehta", contents: "Signed financial statements FY25-26 (3 sets), Board resolution originals, Fixed asset register", purpose: "Statutory audit", mode: "Hand Delivery", location: "Almirah 1, Shelf B", status: "In Custody", receivedByName: "Sneha Iyer", receivedAt: daysFromNow(-18), clientId: nimbus.id, items: [{ name: "Signed financial statements FY25-26 (3 sets)", returned: false }, { name: "Board resolution originals", returned: false }, { name: "Fixed asset register", returned: false }] },
    ...
    { inwardNumber: "IN-2627-005", receivedFrom: "Dr. Meera Bhat", contents: "Form 16, Capital gains statements, LIC premium receipts", purpose: "ITR AY 2026-27", mode: "Post", location: "Tray – current filings", status: "In Custody", receivedByName: "Sneha Iyer", receivedAt: daysFromNow(-5), clientId: meera.id, items: [{ name: "Form 16", returned: false }, { name: "Capital gains statements", returned: false }, { name: "LIC premium receipts", returned: false }] },
  ];
  const packets = [];
  ...
  const itrData = [
    ...
    // The register covers all return types, not only ITR.
    { clientId: apex.id, returnType: "TDS", formType: "Form 138 / 24Q", financialYear: fyCurrent, periodQuarter: "Q1", status: "In Preparation", assigneeId: accountant.id, notes: "Quarterly TDS return on salaries." },
    { clientId: nimbus.id, returnType: "GST", formType: "GSTR-3B", financialYear: fyCurrent, periodMonth: 5, status: "Filed", filedOn: daysFromNow(-25), ackNumber: "AB0605262233445", assigneeId: senior.id, notes: "May 2026 summary return." },
    { clientId: greenleaf.id, returnType: "MCA", formType: "LLP Form 11", financialYear: fyPrev, status: "Filed", filedOn: daysFromNow(-40), ackNumber: "T12345678", assigneeId: article.id, notes: "LLP annual return." },
  ];
