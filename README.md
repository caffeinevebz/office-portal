...
| **Clients** | Full client register (Individual / Proprietorship / Partnership / LLP / Pvt Ltd / HUF / Trust …) with PAN, GSTIN and contacts. Search, filter, create, edit, delete, **bulk import from an Excel template**, and a per-client detail page. Each client can hold **multiple firm / trade names** (a proprietor's several concerns, or a company's brand name) — each with its own GSTIN/PAN/address. Clients can be organised into **groups** with a manually-assigned unique code, filterable across the register. |
