...
## Editing a bill after it has been raised

An invoice can be corrected at any point in its life, including after money has
come in. Saving an edit does not disturb the receipts already taken: a bill
that reads **Partly Paid** can have its wording or its figures corrected and
stays part-paid, and raising the amount on a settled bill leaves it **owing the
difference** rather than inventing a receipt for money nobody paid.

What an edit *cannot* do is quietly point at something that is no longer there.
A client, trade name or billing firm that has since been deleted, or an invoice
number already used by another bill, is **refused with the reason** — never with
"Internal server error". A line whose id has gone stale (the page was open a
while, someone else changed the bill) is kept as what it is: the words that were
