...
| **Receipt Register** | Professional income is accounted on **receipt basis**, so this register lists **money actually received** — every payment by its receipt date — for a **financial year (Apr–Mar), a month, or any custom period**. Columns for gross (incl. GST), **TDS deducted by clients** (claimable) and net received, with period totals, and a **printable register PDF**. |
